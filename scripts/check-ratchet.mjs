...
// 🔴 冻结条目的**目标文件消失**时必须出声。
//
// 上面那个循环遍历的是**磁盘上存在**的文件，于是一个被改名/删除的冻结文件压根不进循环：
// 不报违规、不报变好、条目永远留在 baseline 里。而本仓改名冻结文件是**常规动作**
// （`upstream.rs` 变成目录、`mockData.ts` 拆分片都这么发生过），于是那条死条目会一直
// 计入「N 个冻结文件」这个数字 —— 门每次都报一个与现实脱钩的数，而内容搬去的新文件
// 只受 900 行上限约束。也就是一次**静默的上限放宽**。
//
// 计数判据那侧一直有这个告警（下面那句「已无对应度量」），行数这侧没有 —— 补上对齐。
// 刻意只告警不判违规：改名本身是正当的，判违规会拦住一次合法重构；而告警足以让人
// 想起「搬代码时判据要跟着搬」那条纪律。
for (const f of Object.keys(frozen)) {
  if (!(f in files)) {
    console.log(
      `⚠️  冻结条目 ${f} 在磁盘上找不到（改名或删除了？）——\n` +
        `    它已不再被检查，而内容搬去的新文件只受 ${cap} 行上限约束。\n` +
        `    确认后请手改 ${BASELINE} 删掉这一条，并把新文件按需纳入。`
    );
  }
}
