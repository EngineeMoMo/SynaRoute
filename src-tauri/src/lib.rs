...
///
/// 列表接口刻意剥掉了 trace 正文：日志页每 2s 全量轮询，而单条 trace 的请求/响应体各受
/// `REQ_LOG_CAP`（65536 字符）约束，500 条满载可达数千万字符 —— 每 2s 搬一次会把界面拖卡，
/// 而这些正文只在用户展开某一行时才被看。故列表只带 `hasTrace` 布尔位、展开时按 id 单取。
///
