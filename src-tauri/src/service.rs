...
/// 该分类**当前应当生效**的代理端点。
///
/// 优先取代理实际绑定的端口（首选端口被占时会回退，写错客户端就连不上），
/// 未运行时退回配置里的粘滞端口、再退回分类默认端口。
///
/// 抽出来的理由与 `models_for_apply` 同：这个字符串此前只在「接入」与「改端口」两处
/// 各拼一遍，而现在漂移检测也要用它去比对客户端配置里写着的 base_url。
/// 三处各拼一遍必然漂移，而漂移的表现是「明明配置是对的却报接入失效」这种假警。
pub(crate) fn expected_endpoint(
    store: &Store,
    proxy: &ProxyManager,
    ...
    let port = proxy
        .port_of(category)
        .or_else(|| store.get_settings().proxy_ports.get(&category).copied())
        .unwrap_or_else(|| category.meta().default_port);
...
pub(crate) mod tests {
    ...
    pub(crate) fn temp_store(tag: &str) -> (std::sync::Arc<Store>, std::path::PathBuf) {
