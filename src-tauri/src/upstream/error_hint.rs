...
const THINKING_SIGNATURE_NOTE: &str =
    "\n\n【SynaRoute 说明】这是**扩展思考签名**校验失败，不是密钥或额度问题。\n\
     ...
     注：SynaRoute 从不伪造签名；若本轮还有后续候选、且历史里存在可安全摘除的思考块，\
     会自动摘除并降级为不开思考后重试（届时日志页「故障转移」组有记录）。";
...
mod tests {
    ...
    fn encrypted_reasoning_is_recognised_without_claiming_we_fixed_it() {
        for err in [
            ...
            // 🔴 上一版写 `!note.contains("已自动摘除")`，而那个字面量早已不在任何文案里 ——
            // 断言恒真、什么都没查。真正要守的是：本支若提到「摘除」这个动作，
            // 必须明确它说的是**另一族**，不能让用户以为我们动过加密推理内容。
            if note.contains("自动摘除") {
                assert!(
                    note.contains("与思考块不同"),
                    "提到摘除就必须说清那是思考块那一支做的事，本支什么都没做：{note}"
                );
            }
        }
    }
    ...
    ///
    /// 🔴 **钉性质不钉字面量**：上一版断言 `contains("已自动摘除")`，而那句文案后来被
    /// 如实改成条件句（「**若**本轮还有后续候选、且历史里存在可安全摘除的思考块，**会**
    /// 自动摘除…」）—— 整流只在有后续候选时才发生，无条件说「已自动摘除」在只有一条 Key
    /// 时是假话。改对文案让判据变红，而它想守的东西一条都没破。现在钉的是
    /// 「这一支必须提到摘除这个动作」，陈述句与条件句都接受。
    #[test]
    fn the_two_families_do_not_cross_annotate() {
        let thinking = annotate("THINKING_SIGNATURE_INVALID").unwrap();
        assert!(
            thinking.contains("自动摘除"),
            "思考块那一支要说清我们会对思考块做什么：{thinking}"
        );
        assert!(
        ...
        // 同上：钉正向性质（必须如实说没动它），而不是钉一个已经不存在的字面量的缺席。
        assert!(
            reasoning.contains("不做任何改写"),
            "加密内容我们什么都没做，必须明说 —— 说做过就是界面撒谎：{reasoning}"
        );
    }
