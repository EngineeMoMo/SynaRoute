...
/// **对外契约守卫**（P2-1 目录化的安全绳）。
///
/// 本模块正在被拆成 `upstream/` 下的多个子模块。拆分承诺是「对外的 `crate::upstream::X`
/// 路径一个都不变」—— 外部有 40 多处引用，任何一处路径变了都是破坏性改动。
///
/// 这里把**全部对外名字**逐个引用一遍。少了任何一条 `pub use`，编译立刻失败，
/// 而且**报错点在 upstream 内部**，不必等编译到 proxy.rs 才炸 ——
/// 后者会把「拆分漏了一个 re-export」伪装成「proxy 用了不存在的东西」，误导排查方向。
///
/// 这不是运行期断言：它的全部作���发生在编译期，`#[test]` 只是给它一个存在的理由。
#[cfg(test)]
mod api_surface {
    ...
    fn public_functions_stay_reachable() {
        ...
        let _ = crate::upstream::apply_auth;
        let _ = crate::upstream::parse_openai_tool_call;
        // with_usage 是泛型 async：给足两个类型参数（返回值 T 与 Future 类型）太啰嗦，
        // 直接用一个具体的 future 调用点来证明它可达即可。
