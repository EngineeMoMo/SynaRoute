...
const zh: Dict = {
  ...
  "category.mappingGapTitle": "部分模型缺少故障转移冗余",
  "category.mappingGapSummary": "{count} 个模型不是所有启用 Key 都能服务",
  "category.mappingGapView": "查看",
  "category.mappingGapHint": "模型选择器列出所有启用 Key 可服务模型的并集，所以下列模型都能选到。但它们只有一部分 Key 能服务：那些 Key 全部熔断时，该模型的请求会直接报错，SynaRoute 不会悄悄换成别的模型。去对应 Key 补一条映射，即可让它也能顶上。",
  "category.mappingGapItem": "模型 {expected} 在 {keys} 上缺映射：这些 Key 顶不上它",
...
const en: Dict = {
  ...
  "category.mappingGapTitle": "Some models have no failover coverage",
  "category.mappingGapSummary": "{count} model(s) aren’t servable by every enabled key",
  "category.mappingGapView": "View",
  "category.mappingGapHint": "The model picker lists the union of every enabled key’s servable models, so all models below are selectable. But only some keys can serve them: once those keys are all tripped, requests for that model fail outright — SynaRoute won’t silently swap in a different model. Add a mapping on each listed key to give it coverage.",
  "category.mappingGapItem": "Model {expected} has no mapping on {keys}, so those keys can’t cover it",
