...
const zh: Dict = {
  ...
  "category.mappingGapHint": "下列模型在整个池子里只有一条 Key 能服务。那条 Key 熔断或余额耗尽时，选它的请求会直接报错（SynaRoute 刻意不悄悄换成别的模型）。去别的 Key 补一条映射即可给它冗余。已有两条以上 Key 能服务的模型不在此列 —— 它们已经有冗余了。",
  "category.mappingGapItem": "模型 {expected} 只有「{keys}」能服务",
...
const en: Dict = {
  ...
  "category.mappingGapHint": "Each model below is served by exactly one key in the whole pool. When that key trips or runs out of balance, requests for it fail outright — SynaRoute won’t silently swap in a different model. Add a mapping on another key to give it coverage. Models already served by two or more keys aren’t listed: they have coverage.",
  "category.mappingGapItem": "Model {expected} is served only by “{keys}”",
